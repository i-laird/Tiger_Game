...
void Play_Tiger::play(istream & in) {
    ...
    while(!done) {
        ...
        do {
            ...
            if(!brd.isValid(game_state, tiger_move)){
        ...
        }while(!brd.isValid(game_state, tiger_move));
        game_state[0].location = tiger_move.destination;

        man_move = men_response.next_move(game_state);
        cout << man_move.token.location.row << " " << man_move.token.location.col << " ---> ";
        cout << man_move.destination.row << " " << man_move.destination.col << endl;
